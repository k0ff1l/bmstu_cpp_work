...
namespace bmstu {
    ...
    typedef basic_string<char> string; // а вот однобайтовый char уже не шаблонный, коктретный тип
    typedef basic_string<wchar_t> wstring; // это для широких символов в винде 2 байта на символ (UTF-16), а в линуксе 4 байта (UTF-32)
    typedef basic_string<char8_t> u8string; // это для UTF-8
    typedef basic_string<char16_t> u16string; // это для UTF-16
    typedef basic_string<char32_t> u32string; // это для UTF-32
...
/// Оператор конкатенации строк в новую строку
        friend bmstu::basic_string<T> operator+(const basic_string &left, const basic_string &right) {
            basic_string result;
