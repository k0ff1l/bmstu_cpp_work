...
TEST_F(BmstuListlistTests, initializer_list_strings) {
    using namespace std;
    bmstu::list<std::string> list{"first1"s, "first2"s, "first3"s, "first4"s, "first5"s, "first6"s};
...
TEST_F(BmstuListlistTests, IteratorsPolyForm) {
    ...
    // Преобразование итераторов
    {
        ...
        bmstu::list<int>::ConstIterator const_it(list.begin());
        ASSERT_EQ(const_it, list.cbegin());
        ASSERT_EQ(*const_it, *list.cbegin());

        bmstu::list<int>::ConstIterator const_it1;
